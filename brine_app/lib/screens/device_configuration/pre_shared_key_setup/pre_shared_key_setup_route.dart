...
///
/// The pre-shared key is part of the security configuration for the Brine device. When it makes a request to the cloud
/// backend, the Brine device uses the PSK to create an HMAC signature that is sent with the request. This allows the
/// backend to verify that the request is coming from a legitimate Brine device.
///
/// To avoid compromising the PSK by including it in the Brine device firmware, the PSK is generated by the backend and
/// transferred to the Brine device during the device configuration process.
